...
int32_t tt_get_node_id()
{
	...
	printk("linkaddr: %02x:%02x:%02x:%02x:%02x:%02x\n", _linkaddr[0], _linkaddr[1],
	       _linkaddr[2], _linkaddr[3], _linkaddr[4], _linkaddr[5]);
...
int32_t tt_bind(struct tt_Node *node)
{
	...
	int optval = 1;
	if (setsockopt(node->hal.sock, SOL_SOCKET, SO_REUSEADDR, (const void *)&optval,
		       sizeof(int)) < 0) {
	...
	// optval = 1;
	// if (setsockopt(node->hal.sock, SOL_SOCKET, SO_BROADCAST, (const void*)&optval,
	// sizeof(int)) < 0) {
	...
	if (setsockopt(node->hal.sock, SOL_SOCKET, SO_RCVTIMEO, (const void *)&timeout,
...
int32_t tt_send(struct tt_Node *node, const void *buf, size_t len)
{
	...
	return (int32_t)sendto(node->hal.sock, buf, len, 0, (struct sockaddr *)&addr,
...
int32_t tt_receive(struct tt_Node *node, void *buf, size_t len, uint32_t *ip, uint16_t *port)
{
	...
	int32_t ret =
		(int32_t)recvfrom(node->hal.sock, buf, len, 0, (struct sockaddr *)&addr, &addr_len);
