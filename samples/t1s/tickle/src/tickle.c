						...
						uint8_t receiver)
{
	...
	struct tt_SubmessageHeader *submessage_header =
		(struct tt_SubmessageHeader *)(node->tx_buffer + node->tx_tail);
...
static bool flush_tx(struct tt_Node *node, uint32_t len)
{
	...
	_tt_memmove(node->tx_buffer + sizeof(struct tt_Header), node->tx_buffer + len,
		    node->tx_tail - len);
...
static bool end_encode(struct tt_Node *node, struct tt_SubmessageHeader *submessage_header,
		       bool is_flush)
{
	...
	if (node->tx_tail + roundup <= tt_MAX_BUFFER_LENGTH) { // tail in below the buffer
		...
		node->tx_tail += roundup;
		   ...
		   node->tx_tail + roundup >
			   tt_MAX_BUFFER_LENGTH) { // tail exceeds buffer if roundup
...
static void *decode(struct tt_Node *node, uint8_t *buffer, uint32_t *head, uint32_t tail,
		    uint32_t length)
{
	UNUSED(node);
...
static bool decode_string(struct tt_Node *node, uint8_t *buffer, uint32_t *head, uint32_t tail,
...
{
	_tt_memmove(&node->scheduler[0], &node->scheduler[1],
		    sizeof(struct tt_TCB) * (node->scheduler_tail - 1));
	node->scheduler_tail--;
}
...
int32_t tt_Node_create_client(struct tt_Node *node, struct tt_Client *client,
			      struct tt_Service *service, const char *name,
...
int32_t tt_Node_create_server(struct tt_Node *node, struct tt_Server *server,
			      struct tt_Service *service, const char *name,
...
int32_t tt_Node_create_publisher(struct tt_Node *node, struct tt_Publisher *pub,
				 struct tt_Topic *topic, const char *name)
{
	// TODO: Check dup
...
int32_t tt_Node_create_subscriber(struct tt_Node *node, struct tt_Subscriber *sub,
				  struct tt_Topic *topic, const char *name,
...
static void call_retry(struct tt_Node *node, uint64_t time, void *param)
{
	...
	struct tt_CallRequestHeader *callrequest_header =
		(struct tt_CallRequestHeader *)((void *)submessage_header +
	...
	if (++callrequest_header->retry > client->service->call_retry_count) {
		...
		if (client->service->call_retry_interval == 0) {
			retry_interval =
				client->latency == 0 ? tt_CALL_RETRY_INTERVAL : client->latency;
...
int32_t tt_Client_call(struct tt_Client *client, struct tt_Request *request)
{
	...
	// CallRequestHeader
	struct tt_CallRequestHeader *callrequest_header =
		encode(node, sizeof(struct tt_CallRequestHeader));
	...
	// Make cache
	size_t length =
		ROUNDUP((uintptr_t)node->tx_buffer + node->tx_tail - (uintptr_t)submessage_header);
...
int32_t tt_Publisher_publish(struct tt_Publisher *pub, struct tt_Data *data)
{
	...
	// Header and SubmessageHeader
	struct tt_SubmessageHeader *submessage_header =
		start_encode(node, tt_SUBMESSAGE_TYPE_DATA, tt_SUBMESSAGE_ID_ALL);
	if (submessage_header == NULL) {
		rollback(node, submessage_header);
		return -1;
	}
...
static void node_update(struct tt_Node *node, uint64_t time, void *param)
{
	...
	// Header and SubmessageHeader
	struct tt_SubmessageHeader *submessage_header =
		start_encode(node, tt_SUBMESSAGE_TYPE_UPDATE, tt_SUBMESSAGE_ID_ALL);
	if (submessage_header == NULL) {
	...
	for (int i = 0; i < node->endpoint_count; i++) {
		...
		struct tt_UpdateEntity *update_entity =
			encode(node, sizeof(struct tt_UpdateEntity));
		...
		if (++entity_count == UINT8_MAX) {
			TT_LOG_WARNING(
				"Maximum update entity: endpoint index: %d, endpoint count: %d", i,
				node->endpoint_count);
			break;
		}
	}
...
static bool process_update(struct tt_Node *node, struct tt_Header *header, uint8_t *buffer,
			   uint32_t head, uint32_t tail)
{
	...
	*/
	...
	for (int i = 0; i < entity_count &&
			head + sizeof(struct tt_UpdateEntity) + 2 * sizeof(uint16_t) < tail;
	     i++) {
		struct tt_UpdateEntity *update_entity =
			decode(node, buffer, &head, tail, sizeof(struct tt_UpdateEntity));
			 ...
			 uint32_t head, uint32_t tail)
{
	...
	struct tt_DataHeader *data_header =
		decode(node, buffer, &head, tail, sizeof(struct tt_DataHeader));
	if (data_header == NULL) {
	...
	struct tt_Endpoint *endpoint =
		find_endpoint(node, tt_KIND_TOPIC_SUBSCRIBER, data_header->id);
	if (endpoint != NULL) {
		// Callback
		...
		int32_t decoded = topic->data_decode((struct tt_Data *)data, buffer + head,
						     tail - head, tt_is_native_endian(header));
		sub->callback(sub, data_header->timestamp, data_header->seq_no,
			      (struct tt_Data *)data);
...
static struct tt_SubmessageHeader *get_server_cache(struct tt_Server *server, uint8_t receiver,
						    uint16_t seq_no)
{
	for (int i = 0; i < tt_MAX_SERVER_CACHE_COUNT; i++) {
...
static bool set_server_cache(struct tt_Server *server,
			     struct tt_SubmessageHeader *submessage_header, uint8_t receiver)
{
	size_t length = ROUNDUP((uintptr_t)server->node->tx_buffer + server->node->tx_tail -
				(uintptr_t)submessage_header);
	...
	for (int i = 0; i < tt_MAX_SERVER_CACHE_COUNT; i++) {
		...
		if (cache != NULL && server->cache[i] == NULL) {
			struct tt_CallResponseHeader *callresponse_header =
				(struct tt_CallResponseHeader
					 *)((void *)cache + sizeof(struct tt_SubmessageHeader));
			server->cache[i] = cache;

			struct server_cache_clean_config *clean =
				_tt_malloc(sizeof(struct server_cache_clean_config));
			if (clean == NULL) {
				TT_LOG_ERROR("Out of memory");
				return false;
			}
			...
			if (!tt_Node_schedule(server->node, tt_get_ns() + tt_SERVER_CACHE_TIMEOUT,
...
static bool process_callrequest(struct tt_Node *node, struct tt_Header *header, uint8_t *buffer,
				uint32_t head, uint32_t tail)
{
	UNUSED(node);
	...
	struct tt_Endpoint *endpoint =
		find_endpoint(node, tt_KIND_SERVICE_SERVER, callrequest_header->id);
	if (endpoint == NULL) {
		return true;
	}
	...
	if (submessage_header != NULL) {
		...
		uint32_t decoded =
			service->request_decode((struct tt_Request *)request, buffer + head,
						tail - head, tt_is_native_endian(header));

		uint8_t response[service->response_size];
		int8_t return_code = server->callback(server, (struct tt_Request *)request,
		...
		// CallResponseHeader
		struct tt_CallResponseHeader *call_response_header =
			encode(node, sizeof(struct tt_CallResponseHeader));
		...
		if (return_code == 0) {
			int32_t cdr_len =
				service->response_encode_size((struct tt_Response *)response);
			void *cdr = encode(node, cdr_len);
			if (cdr == NULL) {
				rollback(node, submessage_header);
				return false;
			...
			int32_t encoded_len = service->response_encode(
				(struct tt_Response *)response, cdr, cdr_len);
...
static bool process_callresponse(struct tt_Node *node, struct tt_Header *header, uint8_t *buffer,
				 uint32_t head, uint32_t tail)
{
	struct tt_CallResponseHeader *callresponse_header =
	...
	struct tt_Endpoint *endpoint =
		find_endpoint(node, tt_KIND_SERVICE_CLIENT, callresponse_header->id);
	if (endpoint == NULL) {
		return true;
	}
	...
	if (callresponse_header->return_code == 0) {
		int32_t decoded = service->response_decode((struct tt_Response *)response_buffer,
							   buffer + head, tail - head,
...
static bool process_packet(struct tt_Node *node, uint8_t *buffer, uint32_t head, uint32_t tail)
{
	...
	while (true) {
		...
		TT_LOG_DEBUG("submessage->length: %d / %ld", submessage_header->length, tail - head + sizeof(struct tt_SubmessageHeader));
		...
		if (submessage_header->receiver == tt_SUBMESSAGE_ID_ALL ||
	...
	while (1) {
		...
		if (len == -1) {      // Timeout
			...
			TT_LOG_DEBUG("Process packet from addr: %d.%d.%d.%d:%d len: %d",
				     ...
				     (ip >> BITS_IN_1BYTE) & MASK_8BIT, (ip >> 0) & MASK_8BIT, port,
				     len);
