	...
	while (1) {
		...
		/* Send Control Request Message to Slave Node - Ping */
		{
			...
			case '\0':
				continue;
			default:
				printk("Invalid input: %c. Commands: 1, 2, q, w, a, s, z, x, 3, 4, "
				       "e, r, 0\n",
				       uart_char);
				break;
			}
		...
		if (uart_char == '0' || uart_char == '1' || uart_char == '2' || uart_char == 'q' ||
		    uart_char == 'w' || uart_char == 'a' || uart_char == 's' || uart_char == 'z' ||
		    uart_char == 'x' || uart_char == '3' || uart_char == '4' || uart_char == 'e' ||
		    uart_char == 'r') {
			while (length == 0) {
			...
			if (control_resp_msg->header.msg_type == CONTROL_MSG_TYPE_RESPONSE) {
				...
				printk("Master node doesn't support the control request "
				       "message.\n");
			} else {
...
int initialize(void)
{
	...
	enum PLCA_MODE mode =
		PLCA_MODE_COORDINATOR; /* PLCA_MODE_COORDINATOR or PLCA_MODE_FOLLOWER */
