...
static const struct gpio_dt_spec joint4_limit_button =
	GPIO_DT_SPEC_GET_BY_IDX(BUTTONS_NODE, gpios, 3);
...
static const struct gpio_dt_spec joint5_limit_button =
	GPIO_DT_SPEC_GET_BY_IDX(BUTTONS_NODE, gpios, 4);
...
static const struct gpio_dt_spec joint6_limit_button =
	GPIO_DT_SPEC_GET_BY_IDX(BUTTONS_NODE, gpios, 5);
...
/* My MAC address */
static const uint8_t my_mac_addr[ETH_ALEN] = {
	0xbb, 0xbb, 0xbb, 0xbb, 0xbb, 0xbb,
};
/* Target MAC address */
static const uint8_t target_mac_addr[ETH_ALEN] = {
...
int main(void)
{
	...
	while (1) {
		...
		case CONTROL_MSG_TYPE_REQUEST: {
			...
			send_control_msg_response(
				&t1s_spi_dev, my_mac_addr, target_mac_addr, SLAVE_ID,
				move_joint_ret ? CONTROL_MSG_RESPONSE_SUCCESS
...
void common_move_joint(enum JOINT_NUM joint_num)
{
	...
	/* NOTE:
	 *   - The first delay is the minimum delay required to generate pulses for the stepper
	 * motor.
	 *   - The second delay controls the speed of the stepper motor. (This value differs for
	 * each joint.)
	 *   - (ToDo) For the second delay, an API to calculate the appropriate speed should be
	 * implemented.
	 */
