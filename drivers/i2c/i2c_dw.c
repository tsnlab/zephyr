...
/* DesignWare I2C register offsets (standard DW_apb_i2c) */
...
static void i2c_dw_dump_regs(const struct device *dev, uint32_t reg_base, const char *tag)
{
	struct i2c_dw_dev_config *const dw = dev->data;

	uint32_t con      = sys_read32(reg_base + DW_IC_CON);
	uint32_t tar      = sys_read32(reg_base + DW_IC_TAR);
	uint32_t sar      = sys_read32(reg_base + DW_IC_SAR);
	uint32_t enable   = sys_read32(reg_base + DW_IC_ENABLE);
	uint32_t status   = sys_read32(reg_base + DW_IC_STATUS);

	uint32_t intr_stat = sys_read32(reg_base + DW_IC_INTR_STAT);
	uint32_t raw_intr  = sys_read32(reg_base + DW_IC_RAW_INTR_STAT);
	uint32_t intr_mask = sys_read32(reg_base + DW_IC_INTR_MASK);

	uint32_t abrt  = sys_read32(reg_base + DW_IC_TX_ABRT_SOURCE);
	uint32_t txflr = sys_read32(reg_base + DW_IC_TXFLR);
	uint32_t rxflr = sys_read32(reg_base + DW_IC_RXFLR);

	uint32_t ss_hcnt = sys_read32(reg_base + DW_IC_SS_SCL_HCNT);
	uint32_t ss_lcnt = sys_read32(reg_base + DW_IC_SS_SCL_LCNT);
	uint32_t fs_hcnt = sys_read32(reg_base + DW_IC_FS_SCL_HCNT);
	uint32_t fs_lcnt = sys_read32(reg_base + DW_IC_FS_SCL_LCNT);

	LOG_DBG("\n[DW-I2C DUMP] %s dev=%s reg_base=0x%08x\n", tag, dev->name, reg_base);
	LOG_DBG(" state=0x%02x xfr_len=%u xfr_flags=0x%02x rx_pending=%u\n",
	       dw->state, (unsigned)dw->xfr_len, (unsigned)dw->xfr_flags, (unsigned)dw->rx_pending);
	LOG_DBG(" CON=0x%08x TAR=0x%08x SAR=0x%08x EN=0x%08x STATUS=0x%08x\n",
	       con, tar, sar, enable, status);
	LOG_DBG(" INTR_STAT=0x%08x RAW_INTR=0x%08x INTR_MASK=0x%08x ABRT=0x%08x\n",
	       intr_stat, raw_intr, intr_mask, abrt);
	LOG_DBG(" TXFLR=%u RXFLR=%u SS(H,L)=(%u,%u) FS(H,L)=(%u,%u)\n",
	       (unsigned)txflr, (unsigned)rxflr,
	       (unsigned)ss_hcnt, (unsigned)ss_lcnt, (unsigned)fs_hcnt, (unsigned)fs_lcnt);
	LOG_DBG("[DW-I2C DUMP END]\n\n");
}
			   ...
			   uint16_t slave_address)
{
	...
	while (msg_left > 0) {
		...
		if (ret != 0) {
			i2c_dw_dump_regs(dev, reg_base, "timeout-before-abort");
			...
			if (test_bit_con_master_mode(reg_base)) {
				...
				i2c_dw_dump_regs(dev, reg_base, "timeout-after-abort");
			}
			write_intr_mask(DW_DISABLE_ALL_I2C_INT, reg_base);
			value = read_clr_intr(reg_base);
			ret = -ETIMEDOUT;
...
#if IS_ENABLED(CONFIG_PCIE_CONTROLLER)
#define I2C_DW_PCIE_ALLOC_IRQ(bdf) pcie_get_irq(bdf)
#else
#define I2C_DW_PCIE_ALLOC_IRQ(bdf) pcie_alloc_irq(bdf)
#endif
...
#define I2C_DW_IRQ_CONFIG_PCIE1(n)                                                                 \
	...
	{                                                                                          \
		...
		unsigned int irq = I2C_DW_PCIE_ALLOC_IRQ(dev_cfg->pcie->bdf);                      \
