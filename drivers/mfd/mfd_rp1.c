...
int mfd_rp1_init(const struct device *port)
{
	...
	cfg_addr = data->cfg_addr;

	/* PCIe Setup */
	tmp = sys_read32(cfg_addr + PCIE_RC_PL_PHY_CTL_15);
	tmp &= ~PCIE_RC_PL_PHY_CTL_15_PM_CLK_PERIOD_MASK;
	tmp |= 0x12;
	sys_write32(tmp, cfg_addr + PCIE_RC_PL_PHY_CTL_15);

	tmp = sys_read32(cfg_addr + PCIE_MISC_MISC_CTRL);
	...
	tmp |= (0x1 << PCIE_MISC_MISC_CTRL_MAX_BURST_SIZE_LSB);
	sys_write32(tmp, cfg_addr + PCIE_MISC_MISC_CTRL);
	...
	sys_write32(tmp, cfg_addr + PCIE_MISC_RC_BAR2_CONFIG_LO);
	sys_write32(upper_32_bits(BCM2712_RC_BAR2_OFFSET), cfg_addr + PCIE_MISC_RC_BAR2_CONFIG_HI);

	tmp = sys_read32(cfg_addr + PCIE_MISC_UBUS_BAR2_CONFIG_REMAP);
	tmp |= PCIE_MISC_UBUS_BAR2_CONFIG_REMAP_ACCESS_ENABLE_MASK;
	sys_write32(tmp, cfg_addr + PCIE_MISC_UBUS_BAR2_CONFIG_REMAP);
	...
	tmp = sys_read32(cfg_addr + PCIE_MISC_UBUS_CTRL);
	tmp |= PCIE_MISC_UBUS_CTRL_UBUS_PCIE_REPLY_ERR_DIS_MASK;
	tmp |= PCIE_MISC_UBUS_CTRL_UBUS_PCIE_REPLY_DECERR_DIS_MASK;
	sys_write32(tmp, cfg_addr + PCIE_MISC_UBUS_CTRL);

	sys_write32(0xffffffff, cfg_addr + PCIE_MISC_AXI_READ_ERROR_DATA);
	sys_write32(0x0b2d0000, cfg_addr + PCIE_MISC_UBUS_TIMEOUT);
	sys_write32(0x0aba0000, cfg_addr + PCIE_MISC_RC_CONFIG_RETRY_TIMEOUT);

	tmp = sys_read32(cfg_addr + PCIE_MISC_RC_BAR1_CONFIG_LO);
	tmp &= ~PCIE_MISC_RC_BAR1_CONFIG_LO_SIZE_MASK;
	sys_write32(tmp, cfg_addr + PCIE_MISC_RC_BAR1_CONFIG_LO);

	tmp = sys_read32(cfg_addr + PCIE_MISC_RC_BAR3_CONFIG_LO);
	tmp &= ~PCIE_MISC_RC_BAR3_CONFIG_LO_SIZE_MASK;
	sys_write32(tmp, cfg_addr + PCIE_MISC_RC_BAR3_CONFIG_LO);

	tmp = lower_32_bits(BCM2712_RC_BAR4_PCI);
	tmp &= ~PCIE_MISC_RC_BAR_CONFIG_LO_SIZE_MASK;
	tmp |= encode_ibar_size(BCM2712_RC_BAR4_SIZE);
	sys_write32(tmp, cfg_addr + PCIE_MISC_RC_BAR4_CONFIG_LO);
	sys_write32(upper_32_bits(BCM2712_RC_BAR4_PCI), cfg_addr + PCIE_MISC_RC_BAR4_CONFIG_HI);

	tmp = upper_32_bits(BCM2712_RC_BAR4_CPU) & PCIE_MISC_UBUS_BAR_CONFIG_REMAP_HI_MASK;
	sys_write32(tmp, cfg_addr + PCIE_MISC_UBUS_BAR4_CONFIG_REMAP_HI);
	...
	sys_write32(tmp | PCIE_MISC_UBUS_BAR_CONFIG_REMAP_ENABLE,
		    cfg_addr + PCIE_MISC_UBUS_BAR4_CONFIG_REMAP_LO);

	tmp2 = sys_read16(cfg_addr + BRCM_PCIE_CAP_REGS + PCI_EXP_LNKCTL2);
	tmp = sys_read32(cfg_addr + BRCM_PCIE_CAP_REGS + PCI_EXP_LNKCAP);
	tmp &= ~PCI_EXP_LNKCAP_SLS;
	tmp |= 0x2;
	sys_write32(tmp, cfg_addr + BRCM_PCIE_CAP_REGS + PCI_EXP_LNKCAP);
	tmp2 &= ~0xf;
	tmp2 |= 0x2;
	sys_write16(tmp2, cfg_addr + BRCM_PCIE_CAP_REGS + PCI_EXP_LNKCTL2);

	tmp = sys_read32(cfg_addr + PCIE_RC_CFG_PRIV1_ID_VAL3);
	tmp &= PCIE_RC_CFG_PRIV1_ID_VAL3_CLASS_CODE_MASK;
	tmp |= 0x060400;
	sys_write32(tmp, cfg_addr + PCIE_RC_CFG_PRIV1_ID_VAL3);

	tmp = sys_read32(cfg_addr + PCIE_RC_CFG_VENDOR_VENDOR_SPECIFIC_REG1);
	...
	sys_write32(tmp, cfg_addr + PCIE_RC_CFG_VENDOR_VENDOR_SPECIFIC_REG1);

	/* #PERST */
	tmp = sys_read32(cfg_addr + PCIE_MISC_PCIE_CTRL);
	tmp |= PCIE_MISC_PCIE_CTRL_PCIE_PERSTB_MASK;
	sys_write32(tmp, cfg_addr + PCIE_MISC_PCIE_CTRL);

	/* Enable resources and bus-mastering */
	tmp = sys_read32(cfg_addr + PCI_COMMAND);
	tmp |= (PCI_COMMAND_MEMORY | PCI_COMMAND_MASTER);
	sys_write32(tmp, cfg_addr + PCI_COMMAND);

	/* Assign resources to BARs */
	/* Wait until the register becomes accessible */
	do {
		sys_write32(BCM2712_BAR1_REGION_START,
			    cfg_addr + PCIE_EXT_CFG_DATA + PCI_BASE_ADDRESS_0 + 0x4);
		tmp = sys_read32(cfg_addr + PCIE_EXT_CFG_DATA + PCI_BASE_ADDRESS_0 + 0x4);
	} while (tmp != BCM2712_BAR1_REGION_START);

	sys_write32(BCM2712_BAR2_REGION_START,
		    cfg_addr + PCIE_EXT_CFG_DATA + PCI_BASE_ADDRESS_0 + 0x8);
	sys_write32(BCM2712_BAR0_REGION_START, cfg_addr + PCIE_EXT_CFG_DATA + PCI_BASE_ADDRESS_0);

	tmp = sys_read32(cfg_addr + PCIE_EXT_CFG_DATA + PCI_COMMAND);
	tmp |= PCI_COMMAND_MEMORY;
	sys_write32(tmp, cfg_addr + PCIE_EXT_CFG_DATA + PCI_COMMAND);
...
#define MFD_RP1_INIT(n)                                                                            \
	...
	DEVICE_DT_INST_DEFINE(n, mfd_rp1_init, NULL, &mfd_rp1_data_##n, &mfd_rp1_cfg_##n,          \
			      PRE_KERNEL_1, CONFIG_MFD_INIT_PRIORITY, NULL);
