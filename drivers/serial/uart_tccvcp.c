...
void uart_close (uint8_t channel)
{
    ...
    if (channel < UART_CH_MAX)
    {
        ...
        /* Disable the UART ch */
        sys_write32((uint32_t)NULL, MCU_BSP_UART_BASE + (0x10000UL * (channel)) + UART_REG_CR);
...
static int32_t UART_SetBaudRate (uint8_t ucCh, uint32_t uiBaud) // (uint32_t => int32_t)return type mismatched
{
    uint32_t  u32_div;
    uint32_t  mod;
    uint32_t  brd_i;
...
static void UART_EnableInterrupt (uint8_t ucCh, uint32_t uiPriority, uint8_t ucFIFO, GICIsrFunc fnCallback)
{
    uint32_t  i;
    uint32_t  im = 0UL;
...
static void UART_DmaTxProbe ( uint8_t ucCh, uint32_t * puiAddr)
{
    ...
    if (ucCh < UART_CH_MAX)
    {
        ...
        (void)UART_DmaTxEnable(ucCh, uart[ucCh].sTxDma.iBufSize, (const gdma_information_t *)&uart[ucCh].sTxDma);
    }
}
...
static void UART_DmaRxProbe ( uint8_t ucCh, uint32_t * puiAddr)
{
    ...
    if (ucCh < UART_CH_MAX)
    {
        ...
        (void)UART_DmaRxEnable(ucCh, uart[ucCh].sRxDma.iBufSize, (const gdma_information_t *)&uart[ucCh].sRxDma);
    }
}
        ...
        if (ret != -1) {
            ...
            if (ret != -1) {
                if (uart[ucCh].sOpMode == UART_INTR_MODE) // Configure for interrupt mode
                {
                    ...
                    UART_EnableInterrupt(ucCh, pUartCfg->priority, pUartCfg->fifo, pUartCfg->callback_fn);
...
static int uart_tccvcp_init(const struct device *dev)
{
	...
	uart_pars.port_cfg = 4U;
	uart_pars.fifo = DISABLE_FIFO,                                             \
	uart_pars.stop_bit = TWO_STOP_BIT_OFF;
...
static int uart_tccvcp_configure(const struct device *dev, const struct uart_config *cfg)
{
	...
	return 0;
#else
	struct uart_tccvcp_dev_config *dev_cfg = (struct uart_tccvcp_dev_config *)dev->config;
