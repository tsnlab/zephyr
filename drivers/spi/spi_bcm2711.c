...
static int spi_bcm2711_xfer(const struct device *port)
{
	...
	while (spi_context_tx_buf_on(&data->ctx) || (spi_context_rx_buf_on(&data->ctx) && (cs & SPI_CS_RXD))) {
		/* Tx */
		if (spi_context_tx_buf_on(&data->ctx) && (cs & SPI_CS_TXD)) {
			switch (data->dfs) {
				case DFS_4B:
					...
					break;
				case DFS_2B:
					sys_write32((uint32_t)sys_get_be16(data->ctx.tx_buf), SPI_FIFO(data->base));
					break;
				case DFS_1B:
					sys_write32((uint32_t)*(uint8_t *)data->ctx.tx_buf, SPI_FIFO(data->base));
					break;
			}
...
#define SPI_BCM2711_INIT(n)                                                                         \
	...
	static struct spi_bcm2711_data spi_bcm2711_data_##n = {                                      \
		...
		SPI_CONTEXT_CS_GPIOS_INITIALIZE(DT_DRV_INST(n), ctx)};                             \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(n, spi_bcm2711_init, NULL, &spi_bcm2711_data_##n, &spi_bcm2711_cfg_##n, \
			      POST_KERNEL, CONFIG_SPI_INIT_PRIORITY, &spi_bcm2711_api);
