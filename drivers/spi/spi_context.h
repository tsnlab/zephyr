...
#define SPI_CONTEXT_CS_GPIOS_FOREACH_ELEM(_node_id)                                                \
	DT_FOREACH_PROP_ELEM(_node_id, cs_gpios, SPI_CONTEXT_CS_GPIO_SPEC_ELEM)

#define SPI_CONTEXT_CS_GPIOS_INITIALIZE(_node_id, _ctx_name)                                       \
	._ctx_name.cs_gpios = (const struct gpio_dt_spec[]){COND_CODE_1(DT_SPI_HAS_CS_GPIOS(_node_id),				\
				    ...
				    const struct spi_config *spi_cfg)
{
	...
	bool already_locked = (spi_cfg->operation & SPI_LOCK_ON) &&
			      (k_sem_count_get(&ctx->lock) == 0) && (ctx->owner == spi_cfg);
...
static inline int spi_context_wait_for_completion(struct spi_context *ctx)
{
	...
	if (wait) {
		...
		if (IS_ENABLED(CONFIG_SPI_SLAVE) && spi_context_is_slave(ctx)) {
			...
			timeout_ms = MAX(tx_len, rx_len) * 8 * 1000 / ctx->config->frequency;
 ...
 * with the user data.
 */
static inline void spi_context_complete(struct spi_context *ctx, const struct device *dev,
					int status)
{
#ifdef CONFIG_SPI_ASYNC
	if (!ctx->asynchronous) {
 ...
 */
static inline void *spi_context_get_next_buf(const struct spi_buf **current, size_t *count,
					     size_t *buf_len, uint8_t dfs)
{
					     ...
					     const struct spi_buf_set *rx_bufs, uint8_t dfs)
{
	...
	ctx->tx_buf = (const uint8_t *)spi_context_get_next_buf(&ctx->current_tx, &ctx->tx_count,
								&ctx->tx_len, dfs);

	ctx->current_rx = rx_bufs ? rx_bufs->buffers : NULL;
	ctx->rx_count = ctx->current_rx ? rx_bufs->count : 0;
	ctx->rx_buf = (uint8_t *)spi_context_get_next_buf(&ctx->current_rx, &ctx->rx_count,
							  &ctx->rx_len, dfs);
	...
	// 	ctx->current_tx, ctx->tx_count, ctx->current_rx, ctx->rx_count, (void *)ctx->tx_buf,
	// 	ctx->tx_len, (void *)ctx->rx_buf, ctx->rx_len);
}
 ...
 */
static ALWAYS_INLINE void spi_context_update_tx(struct spi_context *ctx, uint8_t dfs, uint32_t len)
{
	if (!ctx->tx_len) {
		return;
	}
	...
	if (!ctx->tx_len) {
		...
		++ctx->current_tx;
		--ctx->tx_count;
		ctx->tx_buf = (const uint8_t *)spi_context_get_next_buf(
			&ctx->current_tx, &ctx->tx_count, &ctx->tx_len, dfs);
	} else if (ctx->tx_buf) {
		ctx->tx_buf += dfs * len;
	}
}
 ...
 */
static ALWAYS_INLINE void spi_context_update_rx(struct spi_context *ctx, uint8_t dfs, uint32_t len)
{
#ifdef CONFIG_SPI_SLAVE
