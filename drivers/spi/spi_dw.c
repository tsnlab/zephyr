...
static inline uint32_t dw_mmio_read32(const struct device *dev, uint32_t off)
{
	/* DEVICE_MMIO_GET(dev)는 spi_dw가 이미 MMIO_MAP 해둔 VA */
	uintptr_t base = (uintptr_t)DEVICE_MMIO_GET(dev);
	return sys_read32(base + off);
}

static void spi_dw_dump_state_timeout(const struct device *dev)
{
	/*
	 * DW SSI 표준 오프셋(바뀔 가능성 낮음):
	 * CTRLR0  0x00
	 * CTRLR1  0x04
	 * SSIENR  0x08
	 * SER     0x10
	 ...
	 * ICR     0x48
	 */
	uint32_t ctrlr0 = dw_mmio_read32(dev, 0x00);
	uint32_t ctrlr1 = dw_mmio_read32(dev, 0x04);
	uint32_t ssienr = dw_mmio_read32(dev, 0x08);
	uint32_t ser    = dw_mmio_read32(dev, 0x10);
	uint32_t baudr  = dw_mmio_read32(dev, 0x14);
	uint32_t txflr  = dw_mmio_read32(dev, 0x20);
	uint32_t rxflr  = dw_mmio_read32(dev, 0x24);
	uint32_t sr     = dw_mmio_read32(dev, 0x28);
	uint32_t imr    = dw_mmio_read32(dev, 0x2C);
	uint32_t isr    = dw_mmio_read32(dev, 0x30);
	uint32_t risr   = dw_mmio_read32(dev, 0x34);

	LOG_ERR("DW: TIMEOUT DUMP: SSIENR=%u SER=0x%08x SR=0x%08x TXFLR=%u RXFLR=%u",
		ssienr, ser, sr, txflr, rxflr);
	LOG_ERR("DW: TIMEOUT DUMP: CTRLR0=0x%08x CTRLR1=0x%08x BAUDR=%u IMR=0x%08x ISR=0x%08x RISR=0x%08x",
		ctrlr0, ctrlr1, baudr, imr, isr, risr);
}
...
#ifdef SPI_DW_TX_RX_DUMP
	LOG_DBG("push_data: enter f_tx=%d dfs=%d tx_on=%d rx_on=%d",
		f_tx,
		spi->dfs,
		spi_context_tx_on(&spi->ctx),
		spi_context_rx_on(&spi->ctx));
#endif

	while (f_tx) {

		bool from_tx_buf = false;
...
#ifdef SPI_DW_TX_RX_DUMP
		LOG_DBG("SPI TX: data=0x%08x (%s) tx_len=%d tx_count=%d",
			data,
			from_tx_buf ? "TX_BUF" : "DUMMY",
			spi->ctx.tx_len,
			spi->ctx.tx_count);
#endif
...
#ifdef SPI_DW_TX_RX_DUMP
		/* RX dump: limit prints to avoid ISR flooding */
		if (rx_dump_cnt < 64) {
			LOG_DBG("SPI RX: raw=0x%08x dfs=%d fifo_diff=%d",
				data, spi->dfs, spi->fifo_diff);
			rx_dump_cnt++;
		}
#endif
...
static int spi_dw_xfer_polling(const struct device *dev)
{
	...
	while ((k_cycle_get_64() - start) < timeout_cycles) {

		if (spi_context_rx_on(&spi->ctx) && read_rxflr(dev)) {
			pull_data(dev);
		}

		if ((spi_context_tx_on(&spi->ctx) || spi_context_rx_on(&spi->ctx)) &&
		    (read_txflr(dev) < info->fifo_depth)) {
			push_data(dev);
		}
		      ...
		      void *userdata)
{
	...
	/* Make sure controller is disabled while programming CTRLR0/1, thresholds, etc. */
	if (!spi_dw_is_slave(spi)) {
		clear_bit_ssienr(dev);
	}
...
int spi_dw_init(const struct device *dev)
{
...
#ifdef CONFIG_PINCTRL
	err = pinctrl_apply_state(info->pcfg, PINCTRL_STATE_DEFAULT);
	if (err < 0) {
	...
	if (info->clk_dev) {
		...
		if (err < 0) {
			LOG_ERR("Failed to enable the clock (%d)", err);
			return err;
		}
	}
#endif
