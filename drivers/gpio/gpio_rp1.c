...
#define PAD_CTRL(base, n) (base + 0x4 * (n + 1))

#define PAD_OUTPUT_DISABLE 0x80
#define PAD_INPUT_ENABLE   0x40

#define PAD_PULL_UP_ENABLE   0x8
#define PAD_PULL_DOWN_ENABLE 0x4
...
static int gpio_rp1_pin_configure(const struct device *port, gpio_pin_t pin, gpio_flags_t flags)
{
	...
	if (flags & GPIO_OUTPUT) {
		...
		sys_clear_bits(PAD_CTRL(data->pad_base, pin),
			       PAD_OUTPUT_DISABLE | PAD_INPUT_ENABLE);
		...
		sys_set_bits(PAD_CTRL(data->pad_base, pin), PAD_OUTPUT_DISABLE | PAD_INPUT_ENABLE);
	}

	/* Set pull up/down */
	sys_clear_bits(PAD_CTRL(data->pad_base, pin), PAD_PULL_UP_ENABLE | PAD_PULL_DOWN_ENABLE);

	if (flags & GPIO_PULL_UP) {
		sys_set_bits(PAD_CTRL(data->pad_base, pin), PAD_PULL_UP_ENABLE);
	} else if (flags & GPIO_PULL_DOWN) {
		sys_set_bits(PAD_CTRL(data->pad_base, pin), PAD_PULL_DOWN_ENABLE);
	}
...
static const struct gpio_driver_api gpio_rp1_api = {
...
int gpio_rp1_init(const struct device *port)
{
	...
	data->pad_base = DEVICE_MMIO_NAMED_GET(port, reg_base) + config->pad_offset;
...
#define GPIO_RP1_INIT(n)                                                                           \
	...
	static const struct gpio_rp1_config gpio_rp1_cfg_##n = {                                   \
		...
		.pad_offset = DT_INST_REG_ADDR_BY_IDX(n, 2),                                       \
		.ngpios = DT_INST_PROP(n, ngpios),                                                 \
	};                                                                                         \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(n, gpio_rp1_init, NULL, &gpio_rp1_data_##n, &gpio_rp1_cfg_##n,       \
			      PRE_KERNEL_2, CONFIG_GPIO_INIT_PRIORITY, &gpio_rp1_api);
